...
def ollama_chat(message_data, transcribed_text):
    ...
    # data = {'model':'llama3.3', 'messages':[{'role':'user', 'content':"Why is the sky blue?"}]}
    
    # r = requests.post(url, headers=headers, data=json.dumps(data))
    # print(r)

    message_data.append({"role": "user", "content": transcribed_text})
    url = 'http://192.168.86.27:11434/api/generate'
    headers = {'Content-Type': 'application/json'}
    data = {'model':'llama3.1', 'prompt':"Why is the sky blue?"}
